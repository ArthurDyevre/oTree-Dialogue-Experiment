...
class Constants(BaseConstants):
    ...
    if one_choice_per_page:
        if certain_choice:
            num_rounds = num_choices[0]
        else:
            num_rounds = num_choices[0] - 1
    else:
        num_rounds = 1
...
class Player(BasePlayer):
    ...
    if Constants.certain_choice:
        for j in range(1, Constants.num_choices[0] + 1):
            locals()['choice_' + str(j)] = models.StringField()
        del j
    else:
        for j in range(1, Constants.num_choices[0]):
            locals()['choice_' + str(j)] = models.StringField()
        del j
