...
matrix_nature_tau = [[0 for x0 in range(number_of_rounds)] for y0 in range(number_of_students)]
...
def get_payoff_p1(self, sent_amount, sent_back_amount):
    ...
    if tau == 1:
        matrix_nature_tau[i][j] = "hawkish"
    if tau == 2:
        matrix_nature_tau[i][j] = "dovish"

    if sent_amount == 'Exert restrain':
        if pi == 1: # type = hawkish
            return 4
        if pi == 2: # type = dovish
            return 6
    ...
    if sent_amount == 'Be assertive' and sent_back_amount == 'Challenge':
        if tau == 1: # type = hawkish
            return -4
        if tau == 2: # type = dovish
            return -6

# todo: make the new payoffs with nature for player two, by changing parameters to include self and clauses
...
class Results1(Page):
    ...
    def vars_for_template(self):
        ...
        return {
            ...
            'concat_p': zip(np.add(matrix[i], matrix_beliefs[i]), np.add(matrix[i + 1], matrix_beliefs[i + 1]),matrix_toString[i],matrix_toString[i + 1],matrix_beliefs_toString[i + 1],matrix_beliefs_toString[i], matrix_nature[i],matrix_nature_tau[i]),
